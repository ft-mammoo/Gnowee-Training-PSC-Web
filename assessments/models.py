...
class Submission(SoftDeleteModel):
    ...
    status = models.CharField(
        max_length=1, choices=SubmissionStatus.choices, default=SubmissionStatus.SUBMITTED, db_index=True
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                ...
                condition=~models.Q(status="i"),  # Only enforce uniqueness for active records
...
class SubmissionGrade(SoftDeleteModel):
    ...
    def __str__(self):
        return (
            f"Grading {self.id}: Grade {self.grade} for Submission {self.submission_id} by Teacher {self.graded_by_id}"
        )
...
class ExamSubmissions(SoftDeleteModel):
    ...
    student = models.ForeignKey("students.Student", on_delete=models.CASCADE)
    submission_time = models.DateTimeField(auto_now_add=True, db_index=True)
